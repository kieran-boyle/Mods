  ...
  public postDBLoad(container: DependencyContainer):void
  {
    ...
    for (let b in bots)
    {
      ...
      else
      {
        ...
        for (let pocketContents in pockets)
        {
          ...
          if (pockets[pocketContents] === impact && this.config.removeImpact === true)
          {
            pockets.splice(pocketContents, 1)
            impactCount++
          }
        ...
        for (let rigContents in rig)
        {
          ...
          if (rig[rigContents] === impact && this.config.removeImpact === true)
          {
            rig.splice(rigContents, 1)
            impactCount++
          }
    ...
    if (this.config.debug === true)
    {
      ...
      else
      {
        ...
        {
          this.logger.log(`[kiki-ZaryaKiller] : ${impactCount} bot type's RGN's removed`, "yellow", "black")
        }
      }
    }
  }
